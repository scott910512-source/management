...
export default function Hazardous() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(d);
    } catch { setData({ items: [], hazardousItems: [] }); }
  ...
  function downloadCsv() {
    ...
    a.href = url;
    a.download = `유해화학물질관리대장_${year}.csv`;
    a.click();
  ...
  return (
    <>
      <div className="page-head">
        <div className="desc">유해화학물질로 지정된 품목의 연간 수불 이력을 날짜별로 확인합니다.</div>
        <div className="btn-row">
          <select className="plant-select" value={year} onChange={(e) => setYear(Number(e.target.value))}>
            {years.map(y => <option key={y} value={y}>{y}년</option>)}
          </select>
          <button className="btn secondary sm" onClick={downloadCsv} disabled={!data}>⬇ CSV 다운로드</button>
        </div>
      </div>

      {data && data.hazardousItems && data.hazardousItems.length > 0 && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
          <span className="muted" style={{ fontSize: 12, lineHeight: '22px' }}>유해물질 품목:</span>
          {data.hazardousItems.map(n => <Badge key={n} color="red">{n}</Badge>)}
        </div>
      )}

      {loading ? <Loading /> : !data || data.items.length === 0 ? (
        <div className="card card-pad">
          <Empty>유해화학물질로 지정된 품목이 없거나 해당 연도 이력이 없습니다.<br />기준정보에서 품목 등록 시 "유해화학물질" 체크박스를 선택해 주세요.</Empty>
        </div>
      ) : (
