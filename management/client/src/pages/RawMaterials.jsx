...
export default function RawMaterials() {
  const { isAdmin, canWrite, isDemo, demoNum, demoText } = useAuth();
    ...
    <>
      ...
      <div className="page-head">
        ...
        <div className="btn-row">
          ...
          {!isDemo && <button className="btn secondary sm" onClick={exportCsv}>⬇ CSV</button>}
          {isAdmin && <button className="btn secondary sm" style={{ color: 'var(--red)' }} onClick={async () => {
            ...
            try { const r = await api.del('/raw-materials/cleanup/empty'); toast.ok(`소진 Lot ${r.removed}개 정리 완료`); load(); } catch (e) { toast.err(e.message); }
          }}>🗑 소진 Lot 정리</button>}
          {canWrite && <button className="btn secondary sm" onClick={() => setUseOpen(true)}>− 원재료 사용</button>}
          {canWrite && <button className="btn sm" onClick={() => setReceiveOpen(true)}>+ 원재료 입고</button>}
        </div>
      </div>
      ...
      {edit && (
        <RawForm
          ...
          onSaved={() => { setEdit(null); load(); toast.ok('수정했습니다.'); }}
          onError={(m) => toast.err(m)}
        />
      )}
      {tx && (
...
function RawForm({ mode, initial, onClose, onSaved, onError }) {
  ...
  return (
    ...
    {fifo && (
      <FifoInboundConfirm fifo={fifo} onCancel={() => setFifo(null)} onForce={() => { setFifo(null); doSubmit(true); }} />
    )}
    </>
  );
}

function FifoInboundConfirm({ fifo, onCancel, onForce }) {
  return (
    <Modal
      title="⚠ 입고 순서 확인"
      onClose={onCancel}
      footer={<>
        ...
        <button className="btn danger" onClick={onForce}>그대로 입고</button>
      </>}
    >
  ...
  return (
    <>
      ...
      >
        ...
        {type === '출고' && <BatchFields category="raw" materialName={item.itemName} date={txDate} onChange={setBatch} onAutofillQty={(q) => setQuantity(String(q))} />}
        <Field label="비고">
