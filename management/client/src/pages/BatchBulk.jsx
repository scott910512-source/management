...
import { Modal, Select, Loading, Empty, useToast } from '../components/ui';
...
function BulkModal({ products, onClose, onDone }) {
  ...
  const [rows, setRows] = useState([]); // [{ no, qty: {key: number} }]
  ...
  useEffect(() => {
    ...
    api.get(`/batches/bom-stock?product=${encodeURIComponent(product)}&date=${startDate}`)
      .then((d) => {
        ...
        setRows([{ no: String(d.nextNo || 1), qty }]);
      })
  ...
  const submit = async () => {
    ...
    const batches = rows
      ...
      .map((r) => ({ no: r.no, lines: (mats || []).map((m) => ({ category: m.category, name: m.name, quantity: Number(r.qty[key(m)]) || 0 })) }));
    if (batches.length === 0) return toast.error('배치를 입력하세요.');
    if (anyShort) return toast.error('재고가 부족한 자재가 있습니다.');
    setSaving(true);
    try {
      const res = await api.post('/batches/bulk', { product, startDate, txDate: startDate, batches });
  ...
  return (
    <Modal
      title="배치 일괄 처리"
      subtitle="제품(사용처)의 BOM 자재를 여러 배치에 한 번에 출고 — 가장 오래된 Lot부터 FIFO 자동 분배"
      size="lg"
      onClose={onClose}
      footer={
        <>
